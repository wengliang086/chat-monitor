...
sdk_build_dir=/root/build/fast_access_sdk_dir_online
...
svn=http://svn.hoolai.com/access-platform/branches/fast_access_sdk/${!paramsCount}/fast_access_sdk

rm -rf $sdk_command_config_dir
svn co $svn/zzz_conf -r head $sdk_command_config_dir
#channels=($(awk '{print $1}' $sdk_command_config_dir/channel.txt))
lines=0
while read -a ARRAY
do
	aaa=(${ARRAY[@]})
...
uploadOnline(){
	if [ "all" = "$1" ] ; then
           rsync -avzr --progress --exclude '.svn' -e'ssh -p 36000' $sdk_deploy_dir/*  fastsdk@60.28.203.117:/home/fastsdk/deploy_sdk/
    elif [ "$1" = "assets" ] || [ "$1" = "manifest" ] || [ "$1" = "smali" ] || [ "$1" = "application" ] || [ "$1" = "res" ];then
           rsync -avzr --progress --exclude '.svn' -e'ssh -p 36000' $sdk_deploy_dir/$1  fastsdk@60.28.203.117:/home/fastsdk/deploy_sdk/
           rsync -avzr --progress --exclude '.svn' -e'ssh -p 36000' $sdk_deploy_dir/$1.zip  fastsdk@60.28.203.117:/home/fastsdk/deploy_sdk/
    else    
           rsync -avzr --progress --exclude '.svn' -e'ssh -p 36000' $sdk_deploy_dir/$1_fastaccess_sdk.apk  fastsdk@60.28.203.117:/home/fastsdk/deploy_sdk/
    fi
}
...
if [ "all" = "$1" ] ; then
	...
	for((i=0;i<${#channels[@]};i++))
		do
			sh $sdk_command_config_dir/deploy.sh ${channels[i]}
			check ${channels[i]}
		done
	uploadOnline $1
else
	...
	for (( i=1; i<=sdkCount; i++ ))
		do
			sh $sdk_command_config_dir/deploy.sh ${!i}
			check ${!i}
			uploadOnline ${!i}
		done
fi
