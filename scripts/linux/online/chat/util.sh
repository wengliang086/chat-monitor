...
check_port_use() {
	...
	while [ $COUNT -eq 0 ] && [ $WHILE_COUNT -lt $MAX_WHILE_COUNT ]; do
		COUNT=$(lsof -n -i4TCP:$1 | grep LISTEN | wc -l)
		if [ $COUNT -gt 0 ]; then
			echo -e " 端口$1已被占用"
			return 0
		fi
...
check_port_release() {
	...
	echo -e "等待端口$1被释放,等待时间$MAX_WHILE_COUNT 秒\c"
	while [ $COUNT -gt 0 ] && [ $WHILE_COUNT -lt $MAX_WHILE_COUNT ]; do
		COUNT=$(lsof -n -i4TCP:$1 | grep LISTEN | wc -l)
		if [ $COUNT -eq 0 ]; then
			echo " 端口$1被释放"
			return 0
...
start_web() {
	...
	#nohup  $JAVA_HOME/bin/java $JDK_OPTS $ENV_OPTS -Dwebapp_name=$WEBAPP_NAME -Djetty.port=$PORT $jetty_session_work_name -jar $JETTY_HOME/jetty-runner.jar $configfile --lib $JETTY_HOME/lib  --port $PORT  --stop-port $STOP_PORT   --stop-key $STOP_KEY --log $APP_LOGS/$WEBAPP_NAME/http/"$WEBAPP_NAME"_yyyy_mm_dd.log  --path /$WEBAPP_NAME  $WEBAPP_PATH 2>&1 | rotatelogs $APP_LOGS/$WEBAPP_NAME/$WEBAPP_NAME_%Y_%m_%d.logs 86400  &
	nohup $JAVA_HOME/bin/java $JDK_OPTS $ENV_OPTS -Dorg.apache.jasper.compiler.disablejsr199=false -Dwebapp_name=$WEBAPP_NAME -Djetty.logs=$APP_LOGS/$WEBAPP_NAME/http/"$WEBAPP_NAME"_yyyy_mm_dd.log -Djetty.port=$PORT $jetty_session_work_name -jar $JETTY_HOME/jetty-runner.jar $configfile --lib $JETTY_HOME/lib --port $PORT --stop-port $STOP_PORT --stop-key $STOP_KEY --path /$WEBAPP_NAME $WEBAPP_PATH 2>&1 | rotatelogs $APP_LOGS/$WEBAPP_NAME/$WEBAPP_NAME_%Y_%m_%d.logs 86400 480 &
