...
echo "启动$WEBAPP_NAME"
nohup $JAVA_HOME/bin/java $JDK_OPTS $ENV_OPTS -Dwebapp_name=$WEBAPP_NAME -Djetty.logs=$APP_LOGS/$WEBAPP_NAME/http/"$WEBAPP_NAME"_yyyy_mm_dd.log  -Djetty.port=$PORT $jetty_session_work_name -jar $JETTY_HOME/jetty-runner.jar $configfile --lib $JETTY_HOME/lib  --port $PORT  --stop-port $STOP_PORT   --stop-key $STOP_KEY   --path /$WEBAPP_NAME  $WEBAPP_PATH 2>&1 | rotatelogs $APP_LOGS/$WEBAPP_NAME/$WEBAPP_NAME_%Y_%m_%d.logs 86400 480  &
